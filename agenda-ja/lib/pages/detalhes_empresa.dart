...
class _AgendamentoEmpresaDetalhesPageState extends State<AgendamentoEmpresaDetalhesPage> {
  ...
  Future<void> _carregarProfissionais() async {
    ...
    try {
      final profs = await ProfissionalService.instance.listarPorServico(_servico.id);
      if (mounted) {
        setState(() {
  ...
  int _primeiroDiaMes() => DateTime(_mesAtual.year, _mesAtual.month, 1).weekday % 7;
  ...
  Widget build(BuildContext context) {
    if (widget.servico == null) {
      return const Scaffold(body: Center(child: Text('Serviço não informado.')));
    }
                              ...
                              children: [
                                Container(
                                  ...
                                  child: Column(
                                    crossAxisAlignment: CrossAxisAlignment.start,
                                    children: [
                                      Text(_servico.nome, style: const TextStyle(color: Colors.white, fontSize: 18)),
                                      if (_servico.descricao != null) ...[
                                        const SizedBox(height: 8),
                                        Text(_servico.descricao!, style: const TextStyle(color: Colors.white70)),
                                      ],
                                      const SizedBox(height: 8),
                                      Text(
                                ...
                                const Text('Profissional:', style: TextStyle(fontWeight: FontWeight.bold)),
                                const SizedBox(height: 8),
                                      ...
                                      Row(
                                        ...
                                        children: [
                                          ...
                                          Container(
                                            ...
                                            child: Text(_meses[_mesAtual.month - 1], style: const TextStyle(color: Colors.white)),
                                          ),
                                          IconButton(
                                            onPressed: () {
                                              setState(() => _mesAtual = DateTime(_mesAtual.year, _mesAtual.month + 1));
                                ...
                                if (_carregandoHorarios)
                                  ...
                                  const Text('Nenhum horário disponível nesta data.')
                                else
                                  Wrap(
                                    spacing: 10,
                                    runSpacing: 10,
                                    children: _horarios.map((h) {
                                      final sel = _horarioSelecionado?.horario == h.horario;
                                      return GestureDetector(
                                        ...
                                        child: Container(
                                          ...
                                          alignment: Alignment.center,
                                          decoration: BoxDecoration(
                                            color: sel ? Colors.green : const Color(0xFFD9D9D9),
                                            borderRadius: BorderRadius.circular(10),
                                          ),
                                          child: Text(h.horario, style: TextStyle(
                                ...
                                Center(
                                  child: SizedBox(
                                    ...
                                    child: ElevatedButton(
                                      onPressed: _horarioSelecionado == null || _profissionalSelecionado == null
                                          ? null
                                          : () {
                                              Navigator.push(
                                                context,
                                      ...
                                      child: const Text('Confirmar horário', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)),
                                    ),
                                  ),
                                ),
                              ],
